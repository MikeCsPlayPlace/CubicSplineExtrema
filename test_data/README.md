...
The Main() method will read in the dataset file identified by the user, compute the extrema, and list out the computed results. Expected results are captured in files that mirror the input filenames, except that I have appended '.expected_output.csv'. This allows the Main() program to search for that filename to also list out the expected results and indicate the percent error for each xy point.
